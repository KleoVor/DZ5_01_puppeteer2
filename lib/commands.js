module.exports = {
  ...
  checkSiteAvailability: async function (page) {
    try {
        const titleElements = await page.$$('.page-header__title'); // Получить все элементы с классом 'page-header__title'

        if (!titleElements.length) {
            console.log("Элемент с классом 'page-header__title' не найден на странице.");
            return;
        }

        await page.setViewport({ width: 1920, height: 1080 });
    } catch (error) {
        console.error("Произошла ошибка при поиске элементов с классом 'page-header__title':", error);
    }
},
  ...
  chooseRandomMovie: async function (page) {
    ...
    const movieTitle = await randomMovie.$eval("h2", (title) => title.textContent);
  
   
    return movieTitle; // Возвращает название фильма, а не функцию
  },
  ...
  checkElementClassBeforeAndAfterClick: async function(element) {
    const classBeforeClick = await element.evaluate((el) => el.className);
    await element.click();
    const classAfterClick = await element.evaluate((el) => el.className);

    return { classBeforeClick, classAfterClick 
